...
function renderStats() {
  ...
  document.querySelector("#toolsCountStat").textContent = data.metrics?.toolsCount ?? data.docs.length;
  document.querySelector("#toolsLabelStat").textContent = data.metrics?.toolsLabel || "Dokumen";
  document.querySelector("#attendanceStat").textContent = data.attendance.hadir;
  document.querySelector("#attendanceLabelStat").textContent = "Hadir";
  document.querySelector("#schoolYearStat").textContent = data.metrics?.schoolYear || "2024/2025";
  document.querySelector("#semesterStat").textContent = data.metrics?.semester || "Semester Genap";
}
...
function openStudentCountForm() {
  openModal("Edit Jumlah Siswa", `
    ...
    ${exportToolbar("Jumlah Siswa", [["Judul", "Jumlah"], ["Siswa Aktif", data.metrics?.studentCount ?? data.sectionContent.siswa.length]])}
  `);
...
function openToolsCountForm() {
  openModal("Edit Perangkat Ajar", `
    <form class="edit-form" id="toolsCountForm">
      <label>Jumlah perangkat ajar
        <input name="toolsCount" type="number" min="0" value="${escapeHtml(data.metrics?.toolsCount ?? data.docs.length)}" required>
      </label>
      <label>Keterangan
        <input name="toolsLabel" value="${escapeHtml(data.metrics?.toolsLabel || "Dokumen")}" required>
      </label>
      <div class="form-actions"><button class="pink-button" type="submit">Simpan Perangkat Ajar</button></div>
    </form>
    ${exportToolbar("Perangkat Ajar", [["Judul", "Jumlah", "Keterangan"], ["Perangkat Ajar", data.metrics?.toolsCount ?? data.docs.length, data.metrics?.toolsLabel || "Dokumen"]])}
  `);
  document.querySelector("#toolsCountForm").addEventListener("submit", (event) => {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    data.metrics = data.metrics || {};
    data.metrics.toolsCount = Number(form.get("toolsCount"));
    data.metrics.toolsLabel = form.get("toolsLabel").trim();
    saveData();
    renderAll();
    modal.close();
    showToast("Perangkat ajar berhasil diperbarui");
  });
}

function openSchoolYearForm() {
  openModal("Edit Tahun Ajaran", `
    <form class="edit-form" id="schoolYearForm">
      <label>Tahun ajaran
        <input name="schoolYear" value="${escapeHtml(data.metrics?.schoolYear || "2024/2025")}" required>
      </label>
      <label>Semester
        <input name="semester" value="${escapeHtml(data.metrics?.semester || "Semester Genap")}" required>
      </label>
      <div class="form-actions"><button class="pink-button" type="submit">Simpan Tahun Ajaran</button></div>
    </form>
    ${exportToolbar("Tahun Ajaran", [["Tahun Ajaran", "Semester"], [data.metrics?.schoolYear || "2024/2025", data.metrics?.semester || "Semester Genap"]])}
  `);
  document.querySelector("#schoolYearForm").addEventListener("submit", (event) => {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    data.metrics = data.metrics || {};
    data.metrics.schoolYear = form.get("schoolYear").trim();
    data.metrics.semester = form.get("semester").trim();
    saveData();
    renderAll();
    modal.close();
...
function openAttendanceForm() {
  openModal("Edit Absensi", `
    ...
    ${exportToolbar("Absensi Hari Ini", attendanceRows())}
  `);
...
function openManager(section) {
  ...
  openModal(labels[section] || section, `
    ...
    ${exportToolbar(labels[section] || section, rowsForKey(section))}
  `);
}
...
function tableHtml(title, rows) {
  const safeRows = rows.length ? rows : [["Data"], ["Belum ada data"]];
  return `
    <h1>${escapeHtml(title)}</h1>
    <table border="1" cellspacing="0" cellpadding="8">
      ${safeRows.map((row, index) => `
        <tr>${row.map((cell) => index === 0 ? `<th>${escapeHtml(stripTags(cell))}</th>` : `<td>${escapeHtml(stripTags(cell))}</td>`).join("")}</tr>
      `).join("")}
    </table>
  `;
}

function exportToolbar(title, rows) {
  const encoded = encodeURIComponent(JSON.stringify({ title, rows }));
  return `
    <div class="export-toolbar">
      <button type="button" data-export="print" data-payload="${encoded}">Cetak</button>
      <button type="button" data-export="excel" data-payload="${encoded}">Excel</button>
      <button type="button" data-export="pdf" data-payload="${encoded}">PDF</button>
    </div>
  `;
}
    ...
    <html>
      <head>
        ...
        <style>
          ...
          th, td { border: 1px solid #e8b9cb; padding: 9px; text-align: left; }
        </style>
      </head>
      <body>${tableHtml(title, rows)}</body>
    </html>
  `);
  win.document.close();
  win.focus();
...
function exportRows(kind, title, rows) {
  if (kind === "excel") {
    downloadFile(`${slugify(title)}.xls`, `<!doctype html><html><body>${tableHtml(title, rows)}</body></html>`, "application/vnd.ms-excel");
    showToast("File Excel berhasil diunduh");
    return;
  }
  printRows(title, rows);
  showToast(kind === "pdf" ? "Pilih Save as PDF di dialog cetak" : "Dialog cetak dibuka");
}

function rowsForKey(key) {
  if (key === "profile") return profileRows();
  if (key === "attendance") return attendanceRows();
  if (key === "stats") return statsRows();
  if (key === "note") return [["Catatan"], [data.note || ""]];
  if (key === "docs") return [["Judul", "Tipe", "Tanggal"], ...data.docs.map((item) => [item.title, item.type, item.meta])];
  if (key === "activities") return [["Kegiatan", "Waktu"], ...data.activities.map((item) => [item.title, item.meta])];
  if (key === "announcements") return [["Pengumuman", "Tanggal"], ...data.announcements.map((item) => [item.title, item.meta])];
  if (key === "quickItems") return [["Judul", "Deskripsi", "Target"], ...data.quickItems.map((item) => [item.title, item.meta, item.section])];
  if (key === "siswa") return [["Nama Lengkap", "Jenis Kelamin", "Orang Tua", "NISN", "TTL", "Alamat"], ...data.sectionContent.siswa.map((item) => [item.nama, item.jenisKelamin, item.orangTua, item.nisn, item.ttl, item.alamat])];
  if (key === "galeri") return [["Judul Foto", "Keterangan"], ...data.sectionContent.galeri.map((item) => [item.title, item.meta])];
  const list = getList(key);
  return [["Judul", "Keterangan"], ...list.map((item) => [item.title || item.nama || "", item.meta || ""])];
}

function profileRows() {
  return [["Kolom", "Isi"], ...Object.entries(data.profile).map(([key, value]) => [key, value])];
}

function attendanceRows() {
  return [["Status", "Jumlah"], ["Hadir", data.attendance.hadir], ["Sakit", data.attendance.sakit], ["Izin", data.attendance.izin], ["Alpha", data.attendance.alpha]];
}

function statsRows() {
  return [
    ["Kartu", "Nilai", "Keterangan"],
    ["Jumlah Siswa", data.metrics?.studentCount ?? data.sectionContent.siswa.length, "Siswa Aktif"],
    ["Perangkat Ajar", data.metrics?.toolsCount ?? data.docs.length, data.metrics?.toolsLabel || "Dokumen"],
    ["Absensi Hari Ini", data.attendance.hadir, "Hadir"],
    ["Tahun Ajaran", data.metrics?.schoolYear || "2024/2025", data.metrics?.semester || "Semester Genap"]
  ];
}

function addExportButtons() {
  const configs = [
    ["guru", "Data Guru", "profile"],
    ["kalender", "Kalender Kegiatan", "announcements"],
    ["absensi", "Absensi Hari Ini", "attendance"],
    ["laporan", "Grafik Kehadiran", "stats"],
    ["akses-cepat", "Akses Cepat", "quickItems"],
    ["modul", "Dokumen Terbaru", "docs"],
    ["catatan", "Catatan Cepat", "note"],
    ["jurnal", "Kegiatan Terbaru", "activities"]
  ];

  configs.forEach(([id, title, key]) => {
    const panel = document.querySelector(`#${id}`);
    const head = panel?.querySelector(".panel-head");
    if (!head) return;
    let wrap = head.querySelector(".export-mini");
    if (!wrap) {
      wrap = document.createElement("div");
      wrap.className = "export-mini";
      head.appendChild(wrap);
    }
    wrap.innerHTML = exportToolbar(title, rowsForKey(key));
  });
}
...
document.addEventListener("click", (event) => {
  ...
  } else if (action === "edit-tools-count") {
    openToolsCountForm();
  } else if (action === "edit-school-year") {
    openSchoolYearForm();
...
document.addEventListener("click", (event) => {
  ...
  event.stopPropagation();
  const payload = JSON.parse(decodeURIComponent(exportButton.dataset.payload));
  exportRows(exportButton.dataset.export, payload.title, payload.rows);
});
